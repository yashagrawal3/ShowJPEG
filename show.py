...
DEFAULT_BOOKS = "/home/olpc/Pictures/*.jpg"
...
class Photo:
	...
	#   We use whichever is smaller
        ...
        newImage = pygame.transform.smoothscale( image,
	     ( int( float( imageRect.width ) * scale),
	       int( float( imageRect.height ) * scale)) )
	...
	#  Now add the meta (text) information
#	font = pygame.font.Font( None, FONT_SIZE )
#	title_text = font.render( self.title, True, title_color )
#	desc_text = font.render( self.desc, True, desc_color )
#	date_text = font.render( self.date, True, date_color )
#	title_textRect = title_text.get_rect()
#	title_textRect.topleft = ( TITLE_BAR_LEFT_X, TITLE_BAR_TOP_Y )
#	desc_textRect = desc_text.get_rect()
#	desc_textRect.topleft = ( DESC_BAR_LEFT_X, DESC_BAR_TOP_Y )
#	date_textRect = date_text.get_rect()
#	date_textRect.topright = ( DATE_BAR_RIGHT_X, DATE_BAR_TOP_Y )

#	time_text = font.render( "%2.2f KB/s" % \
#		  (float( image_size ) / float( self.download_time )),
#		  True, title_color )
#	time_textRect = time_text.get_rect()
#	time_textRect.topright = ( DATE_BAR_RIGHT_X, DESC_BAR_TOP_Y )
#	screen.blit( time_text, time_textRect)

#	screen.blit( title_text, title_textRect)
#	screen.blit( desc_text, desc_textRect)
#	screen.blit( date_text, date_textRect)
...
class MediaArchive:
	...
	filelist = glob.glob( DEFAULT_BOOKS )
	filelist.sort()
	...
	curr = self.cur_photo
	curr += 1
	if curr >= self.max_history:  curr = 0
        self.cur_photo = curr
	...
	if self.cur_photo == self.tail:
	   ...
	   self.cur_photo -= 1
	   if self.cur_photo < 0:  self.cur_photo = self.max_history - 1
	return self.history[ self.cur_photo ]
	...
	if self.head == self.tail:
           self.tail += 1
           if self.tail >= self.max_history:  self.tail = 0
	return
